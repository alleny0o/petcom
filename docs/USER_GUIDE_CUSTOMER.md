...
- **A small blue dot** next to an order means it has been updated since
  you last looked.
...
*The registration form. Pick your institute first — the lab list fills in based on your institute, and the PI list fills in based on your lab.*

Fill in every field:

1. **Institute** — your NIH institute or center.
2. **Lab** — your lab (the list shows labs at the institute you picked).
   If your lab isn't listed, contact an administrator.
3. **First name** and **Last name**.
4. **Email** — must be your **@nih.gov** address. This becomes your
   username for logging in.
5. **Phone** — a number where you can be reached.
6. **Supervising PI** — the principal investigator you work under (the
   list shows PIs at the lab you picked). If your PI isn't listed,
...
*The status page tells you whether your registration is Pending, Approved, or Rejected.*

- **Pending** — an administrator hasn't reviewed it yet.
- **Approved** — you're in; your login details come from an administrator
  via NIH email. If you haven't received them, contact an administrator.
- **Rejected** — your request wasn't approved. Contact an administrator
  for details. You're welcome to submit a new registration.
...
*Set your real password here. You'll use it from now on.*

Your new password must be **at least 12 characters** and include **at
least one letter and one number**. It can't contain your username, and it
can't be one you've used recently.

Two things to know about logging in:

- If you see "Invalid username or password," check both carefully and try
  again.
- After several wrong attempts in a row, logins for your account stop
  ...
  sure your password is correct but it isn't working, wait 15 minutes and
  try again.
...
*The dashboard: your lab's order counts on top, recent orders below, and your lab info and quick links on the right.*

- **Stat tiles** — Pending Orders, Requested This Month, and Total Orders
  for your whole lab (click one to jump to that filtered list), plus a
  count of products currently available to order.
- **Recent Orders** — your lab's five most recent orders. Click **View**
  to open one.
- **My Lab** — your lab and institute, with a **View full info** button
  showing your full account details.
- **Quick Links** — shortcuts to Delivery Locations and Product Users
  (sections 10 and 11).
...
*The New Order form. Fields appear in the order you fill them: nuclide first, then product, then the details.*
...
2. **Product** — pick the product. Each option shows how it's fulfilled
   (Radiopharmacy, Pick Up, or Direct Delivery). Fulfillment is fixed per
   product — if the same product appears twice, that's two fulfillment
   options; pick the one you need.
3. **Delivery location** — this field only appears when you've picked a
   **Direct Delivery** product. Choose where in your lab the dose should
   go. If the list is empty, there's a link to add a location first (see
   section 10).
   ...
   *The Delivery section appears only for Direct Delivery products.*

4. **Activity (mCi)** — the requested activity.
5. **Requested date** — pick the date.
6. **Requested time** — type the time in **24-hour format**, like
   `14:30` for 2:30 PM.
...
*One last check before the order is submitted.*
...
If you close the form partway through, you'll be asked whether to discard
what you entered, so you won't lose work to a stray click.
...
*The Orders page: status tabs across the top, search and filters below, and the order list underneath.*

- **Status tabs** — All, Pending, Accepted, Completed, Cancelled, each
  with a count. Click a tab to filter.
- **Search box** — find orders by order number, product, nuclide, or
  product user name. Type your search and click **Filter**.
- **Fulfillment filter** — narrow to Radiopharmacy, Pick Up, or Direct
  Delivery orders.
- **Date range** — show orders requested between two dates.

Click **View** on any row to open the order. If the list is long, use the
page controls at the bottom.

![Order detail page showing the order's full details](images/customer/order-detail.png)
*An order's detail page: status at the top, full order details below, and a Print button for a paper copy.*

## 7. Editing a pending order

Open one of **your own** orders while it's still **Pending** and you'll
see an **Edit Order** button on the details card. (Orders your lab-mates
placed, and orders that staff have already accepted, can't be edited —
if something needs to change on those, put it in the Notes or contact
staff.)

![Order detail page in edit mode](images/customer/order-edit.png)
*Editing a pending order — the same fields as the order form.*
...
Every order has a **Notes** card. On your own **pending** orders you can
edit it: type your note (up to 500 characters) and click **Save Notes**.

![Notes card on an order with the save button](images/customer/order-notes.png)
*Notes are shared between you and the staff processing the order.*
...
You can cancel **your own** orders while they're **Pending**. Open the
order and click **Cancel Order** in the top-right.

![Cancel order dialog asking for a reason](images/customer/order-cancel-modal.png)
*Cancelling always requires a reason.*

Type the reason for cancelling (required) and click **Cancel Order**, or
**Keep Order** to back out. The order moves to **Cancelled**, and the
reason is saved on the order for everyone to see.

If you cancel by mistake, contact staff — they can reopen a cancelled
order.

Once staff have **accepted** an order, you can no longer cancel it
yourself — contact staff and they can cancel it for you (staff will also
record a reason).
...
*Your lab's delivery locations. Only Active locations can be picked on new orders.*

- **Add:** click **+ Add Location**, enter a **Name** (required) and
  **Room** (optional), and click **Add Location**.

  ![Add delivery location dialog](images/customer/delivery-location-add-modal.png)
  *Adding a delivery location — just a name and an optional room.*

- **Edit:** click Edit on a row to change its name or room.
- **Deactivate / Activate:** deactivating a location removes it from the
  choices on *new* orders — it doesn't touch past orders, and nothing is
  ever deleted. You can reactivate it any time.
...
*Your lab's product users. Only Active product users can be picked on new orders.*

- **Add:** click **+ Product User** and enter their **First name**,
  **Last name**, and **Email** (all required — and each email can only be
  used once in your lab's list).
...
- **Deactivate / Activate:** deactivating removes them from the recipient
  choices on *new* orders, without affecting past orders. Reactivate any
  time.
...
*My Info is view-only. To change any of these details, contact an administrator.*
...
Password** (linked from My Info). You'll need your current password, and
the new one follows the same rules as in section 3.
