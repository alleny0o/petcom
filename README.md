...
orders for products by nuclide; PET Department staff process those orders
through a simple lifecycle (pending → accepted → completed); and
...
- **Customer** — a lab member: places orders, tracks their lab's orders,
  and maintains the lab's delivery locations and product users.
- **Staff** — processes any order from any lab: accept, complete,
  return, cancel, reopen, plus notes and the chargeable flag.
- **Admin** — everything staff can do, plus registration approval,
  account management, catalog and directory management, and CSV reports.

## Documentation

| Document | Audience | What's in it |
|---|---|---|
| [Deployment Guide](docs/DEPLOYMENT.md) | IT / sysadmin | Step-by-step first-time production deployment on RHEL 8 — prerequisites through verification checklist. |
